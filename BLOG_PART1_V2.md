...
│  ┌──────────────────────────────────────────────────────────────┐  │
│  │                    前端（用户接口层）                         │  │
│  │   OpenAI Compatible API  /  AsyncLLMEngine  /  LLMEngine     │  │
│  │   接收请求 → 异步队列 → 流式返回结果                          │  │
│  └──────────────────────┬───────────────────────────────────────┘  │
│                         │ (IPC 通信，独立进程)                      │
│  ┌──────────────────────▼───────────────────────────────────────┐  │
│  │                    EngineCore（核心引擎，独立进程）            │  │
│  │                                                              │  │
│  │  ┌─────────────────────────────────────────────────────┐    │  │
│  │  │  Scheduler（调度器）                                │    │  │
│  │  │  ├── waiting 队列（新请求）                         │    │  │
│  │  │  ├── running 队列（正在推理）                       │    │  │
│  │  │  ├── KV Cache Manager（块分配/释放/Prefix Cache）   │    │  │
│  │  │  └── schedule() → SchedulerOutput                  │    │  │
│  │  └──────────────────────┬──────────────────────────────┘    │  │
│  │                         │ SchedulerOutput                    │  │
│  │  ┌──────────────────────▼──────────────────────────────┐    │  │
│  │  │  ModelExecutor（模型执行器）                        │    │  │
│  │  │  ├── 准备 InputBatch（token ids, positions, slots）  │    │  │
│  │  │  ├── GPU Worker 执行 forward pass                   │    │  │
│  │  │  │   ├── Embedding                                  │    │  │
│  │  │  │   ├── Attention（FlashAttention + PagedAttn）    │    │  │
│  │  │  │   ├── FFN / MoE                                  │    │  │
│  │  │  │   └── LM Head → logits                          │    │  │
│  │  │  └── Sampler（贪心 / Top-P / Spec Decode）          │    │  │
│  │  └──────────────────────┬──────────────────────────────┘    │  │
│  │                         │ ModelRunnerOutput                  │  │
│  │  ┌──────────────────────▼──────────────────────────────┐    │  │
│  │  │  Scheduler.update_from_output()                     │    │  │
│  │  │  更新请求状态 / 释放完成请求的 KV Cache块            │    │  │
│  │  └─────────────────────────────────────────────────────┘    │  │
│  └──────────────────────────────────────────────────────────────┘  │
│                                                                     │
│  ┌──────────────────────────────────────────────────────────────┐  │
...
│  │  ┌──────────┐  ┌───────────────────────────────────────┐   │  │
│  │  │ 模型权重  │  │       KV Cache 块池                   │   │  │
│  │  │ ~60-80%  │  │  [Block0][Block1][Block2]...[BlockN]  │   │  │
│  │  │          │  │       ~20-40% 显存                     │   │  │
│  │  └──────────┘  └───────────────────────────────────────┘   │  │
│  └──────────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────────────┘
...
t=0  用户发送请求 "你好"
      │
      ▼
t=1  AsyncLLMEngine 接收，生成 Request 对象
     Request {
       prompt_token_ids: [1, 2345, 567],  ← tokenizer 编码
       max_new_tokens: 100,
       status: WAITING,
     }
      │
      ▼
t=2  Scheduler.schedule()
     - 从 waiting 队列取出请求
     - KVCacheManager 分配物理块
     - 返回 SchedulerOutput { num_scheduled_tokens: {req_id: 3} }
      │
      ▼
t=3  GPU Worker.execute_model()
     Prefill Pass:
     - 输入：token_ids=[1,2345,567], positions=[0,1,2]
     - 计算所有 token 的 K, V，写入 KV Cache
     - 输出：logits[2]（最后一个token的logit）→ 采样 → token "你"
      │
      ▼
t=4  Scheduler.update_from_output()
     - num_computed_tokens: 3 → 4
     - output_token_ids: [你的token_id]
      │
      ▼
t=5  Scheduler.schedule()（第2步）
     - 请求已完成 Prefill，进入 Decode 阶段
     - 分配新的 KV Cache 槽位
      │
      ▼
t=6  GPU Worker.execute_model()
     Decode Pass:
     - 输入：token_ids=[你的token_id], positions=[3]
     - 从 KV Cache 读取历史 K,V，计算新 attention
     - 输出：logits → "好"
      │
      ▼
...
V1 架构（异步，多进程）:
  前端进程                  EngineCore 进程
  AsyncLLMEngine  ←IPC→  Scheduler + Executor
  (非阻塞 API)            (独立循环，连续 batch)
...
```
OS 虚拟内存分页：
  进程A看到的虚拟地址空间：[0x0000 ─────────── 0xFFFF]
  实际映射到物理内存的页：  [ P3 ][ P7 ][ -- ][ P1 ]

  优势：
  1. 进程无需关心物理内存布局
  2. 非连续物理内存可以组成连续的虚拟地址空间
  3. 内存可以按需分配（缺页中断）
  4. 多个进程可以共享同一物理页（fork/COW）
```
...
```
PagedAttention 的"分页"：
  逻辑 KV 序列（请求视角）：[K0,V0][K1,V1][K2,V2][K3,V3]...
  物理 KV 块（GPU 显存）：    [ Block7 ][ Block2 ][ Block15 ]...
  映射关系（Block Table）：   [0→7]    [1→2]    [2→15]
...
│  └────────────────────────────────────────────────┘   │
│  方法：allocate() / free() / write_kv() / read_kv()   │
└───────────────────────────┬────────────────────────────┘
                            │
              ┌─────────────▼─────────────┐
              │       Request State        │
...
KV Cache 块的状态机：
          ...
          allocate()
POOL ────────────────→ ALLOCATED（已分配给某请求）
  ↑                         │
  │     free()              │  request finished
  │  (ref_count→0)          │
  └──────────────── CACHED  │
       mark_cached()   ←────┘
          (LRU候选池)
...
┌─────────────────────────────────────────────────────────┐
│  block_pool: dict[block_id → Block]                     │
│    每个 Block 记录：ref_count, hash, prev, next         │
│                                                         │
│  cached_blocks: dict[hash → Block]                      │
│    Prefix Cache：hash → 物理块（快速查找）              │
│                                                         │
│  LRU Free Queue（双向链表）：                           │
│    HEAD ↔ [最旧的空闲块] ↔ ... ↔ [最近释放的块] ↔ TAIL │
│    驱逐时从 HEAD 取                                     │
│                                                         │
│  总体不变量：                                           │
...
│                                                                 │
│  t=0  Request 到达                                              │
│       prompt_token_ids = [101, 234, 567, ..., 892]  (12 tokens)│
│       → 进入 waiting 队列                                       │
│                                                                 │
│  t=1  Scheduler.schedule()  [第1步]                             │
│       ├── 从 waiting 取出 Request                              │
│       ├── 检查 Prefix Cache：MISS（新请求）                    │
│       ├── 分配物理块：blocks=[#5]（block_size=16，12<16，1块够）│
│       ├── slot_mapping = [0,1,2,...,11]（slots 0-11 in block 5）│
│       └── SchedulerOutput {                                    │
│             scheduled: [Request],                              │
│             num_tokens: {req_id: 12},                          │
│             is_prefill: True                                   │
│           }                                                    │
│                                                                 │
│  t=2  GPU Worker: Prefill Forward Pass                          │
│       输入：token_ids=[101,234,...,892], positions=[0..11]      │
│       每层 Transformer：                                        │
│       ├── Embedding：[12, hidden_dim]                          │
│       ├── Attention：                                           │
│       │   ├── 计算 Q, K, V：各 [12, num_heads, head_dim]      │
│       │   ├── 写入 KV Cache：k_cache[5][0:12], v_cache[5][0:12]│
│       │   └── Causal Attention：[12, num_heads, head_dim]     │
│       └── FFN：[12, hidden_dim]                                │
│       输出：logits[11]（最后一个token的logit） → sample → T0  │
│                                                                 │
│  t=3  Scheduler.update()                                        │
│       num_computed: 12 → 13                                    │
│       output_tokens: [T0]                                      │
│                                                                 │
│  ── 进入 Decode 循环（重复 50 次）──                            │
│                                                                 │
│  t=4  Scheduler.schedule() [第2步]                              │
│       ├── 已完成 Prefill，进入 Decode                          │
│       ├── 分配新 slot：block[5] 中的 slot=12                   │
│       └── slot_mapping = [12]                                  │
│                                                                 │
│  t=5  GPU Worker: Decode Forward Pass                           │
│       输入：token_ids=[T0], positions=[12]                     │
│       每层 Transformer：                                        │
│       ├── Embedding：[1, hidden_dim]                           │
│       ├── Paged Attention（Decode）：                          │
│       │   ├── 计算新 Q：[1, num_heads, head_dim]              │
│       │   ├── 写入新 K, V：k_cache[5][12], v_cache[5][12]     │
│       │   ├── 从 KV Cache 读取历史 K,V：slots 0-12            │
│       │   └── Attention 输出：[1, num_heads, head_dim]         │
│       └── 输出：logits → sample → T1                           │
│                                                                 │
│  t=6  ...（重复 Decode，当 token 数超出 block[5] 时分配新块）  │
│                                                                 │
│  当生成第 16+ 个 token 时：                                    │
│       block[5] 满了（16 slots 已用完）                         │
│       Scheduler 分配新块 block[#11]                           │
│       block_table: [5] → [5, 11]                               │
│                                                                 │
│  t=N  生成 EOS token，请求完成                                 │
│       释放 block[5] 和 block[11]                               │
│       它们进入 LRU 队列（等待被驱逐或 Prefix Cache 复用）      │
└─────────────────────────────────────────────────────────────────┘
```
...
────────────────────────────────────────────────────
请求A    [PF3]  [D]    [D]    [D]    [D]    DONE
请求B    [PF5]  [PF5]  [D]    [D]    DONE
请求C           [PF2]  [D]    [D]    [D]    [D]    ...

批次token数：
  Step1:  3+5    = 8   tokens  (A prefill 3, B prefill 5)
  Step2:  1+5+2  = 8   tokens  (A decode 1, B prefill 5, C prefill 2)
  Step3:  1+1+1  = 3   tokens  (A,B,C all decode)
  Step4:  1+1+1  = 3   tokens
  ...

注：
  PF3 = Prefill（3 tokens）
  D   = Decode（1 token）
  新请求（C）在 Step2 插入，无需等待 A, B 完成
```
